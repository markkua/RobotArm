...
def match_template(template_img, draw_img, target_kp, target_descrip, threshold, min_match_count=25):
	...
	kp = orb.detect(template_img, None)
	
	# compute the descriptors with ORB
	template_kp, template_des = orb.compute(template_img, kp)
...
def test_match_thread(target_img, temp_img_ls):
	...
	orb = cv2.ORB_create()
	# find the keypoints with ORB
	kp = orb.detect(target_img, None)
	
	# compute the descriptors with ORB
	target_kp, target_des = orb.compute(target_img, kp)
	
	print('kp', target_kp)
	print('des', target_des)
